...
function CheckLevelQuizzData(page, array) {
  if (array[page] === null || array[page] === undefined) {
    alert(
      "Voce precisa preencher as informacoes de todos os niveis antes de continuar"
    );
    return false;
  } else {
...
function GenerateUserRequestPost(questions, levels) {
  ...
  requestUserQuizz.then((response) => {
    ...
    document
      .querySelector(".quizzesUser")
      .setAttribute("onclick", `chamarUmQuizz(${response.data.id})`);
    console.log("opa");
  });
...
function RenderUserQuizz(idQuizz) {
  let responseQuizz = axios.get(
    `https://mock-api.driven.com.br/api/v6/buzzquizz/quizzes/${idQuizz}`
  );
  responseQuizz.then((response) => {
    ...
    document.querySelector(".user-quizz-container").innerHTML += `
        <div class="all-quizz-box" id = "${idQuizz}" onclick="chamarUmQuizz(${idQuizz})") data-identifier="quizz-card">
