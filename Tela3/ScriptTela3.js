...
let page1, quizzInfo, quizzQuestions, quizzLevels, quizzSuccess;
...
function RenderAllQuizzes(quizzes) {
    ...
    allquizzcontainer.innerHTML =
      '<div class = "all-quizz-container-p"><p>Todos os Quizzes</p></div>';
...
page1.innerHTML=`
<section class="quizz-questions escondido">

    <div class="quizz-questions-title">
        <h2> Crie suas perguntas </h2>
    </div>

    <div class="quizz-questions-input-container"> 
...
function changeToLevels(){
    ...
    page1 = document.querySelector('.Tela1');
    page1.innerHTML=`
    <section class="quizz-levels escondido">

    <div class="quizz-levels-title">
        <h2> Agora, decida os níveis! </h2>
    </div>

    <div class="quizz-levels-input-container">
...
function changeToSuccess(){
    ...
    page1 = document.querySelector('.Tela1');
    page1.innerHTML=`
    <section class="quizz-success escondido">
         
        <div class="quizz-success-title">
            <h2> Seu quizz está pronto! </h2>
        </div>

        <div class="quizz-success-container">

            <div class="quizz-success-box"></div>

            <div class="quizz-success-btn">
                <button onclick="getUserQuizzDate('acess')"> Acessar Quizz </button>
            </div>
...
function GetUserQuizzData(page) {
    ...
    <section class="Tela3">
        ...
        <section class="quizz-info">

            <div class="quizz-info-title">
                <h2> Comece pelo começo </h2>
            </div>
            
            <div class="quizz-info-input-container">
                <input class="user-quizz-title" type="text" placeholder="Título do seu quizz" />
                <input class="user-quizz-url" type="text" placeholder="URL da imagem do seu quizz" />
                <input class="user-quizz-questions-amount" type="number" placeholder="Quantidade de perguntas do quizz" />
                <input class="user-quizz-levels-amount" type="number" placeholder="Quantidade de níveis do quizz" />

                <div class="quizz-info-button-box">
                    <button onclick="GetUserQuizzData('questions')">
...
if (page === "info") {
  ...
  if (page === "questions") {
    ...
    let quizzQuestionsAmount = document.querySelector(
      ".user-quizz-questions-amount"
    ).value;
    let quizzLevelsAmount = document.querySelector(".user-quizz-levels-amount")
      .value;
...
function RenderQuestions(number, page) {
  const QuestionsInputContainer = document.querySelector(
    ".quizz-questions-input-container"
  );
...
function CheckInfoQuizzData(title, url, questions, levels) {
  if (title.length < 20 || title.length > 60) {
    return alert(
      "O título do quizz deve ter no mínimo 20 e no máximo 65 caracteres"
    );
  }
  if (!CheckURL(url)) {
...
function CheckQuestionsQuizzData(page, array) {
  if (array[page] === null || array[page] === undefined) {
    alert(
      "Voce precisa preencher as informacoes de todas as perguntas antes de continuar"
    );
    return false;
  } else {
    if (
      array[page].quizzQuestion === "" ||
      array[page].quizzQuestion.length < 20
    ) {
      return alert(
        `O texto da pergunta ${page + 1} deve ter no mínimo 20 caracteres`
      );
    }
    if (CheckHexa(array[page].quizzQuestionColor)) {
      return alert(
        `A cor de fundo da pergunta ${
          page + 1
        } deve ser uma cor em hexadecimal comecando com #`
      );
    }
    if (array[page].quizzRightAnswer === "") {
      return alert(
        `A resposta da pergunta ${page + 1} nao pode ficar em branco`
      );
    }
    if (!CheckURL(array[page].quizzRightAnswerURL)) {
      return alert(
        `A imagem da resposta da pergunta ${page + 1} deve ter formato de URL`
      );
    }

    if (array[page].quizzIncorrectAnswer1 === "") {
      return alert(
        `A pergunta ${page + 1} deve ter no mínimo uma resposta errada`
      );
    }
    if (!CheckURL(array[page].quizzIncorrectAnswerURL1)) {
      return alert(
        `A imagem da resposta incorreta da pergunta ${
          page + 1
        } deve ter formato de URL`
      );
    }
    ...
    if (array[page].quizzIncorrectAnswer2 !== "") {
      if (!CheckURL(array[page].quizzIncorrectAnswerURL2)) {
        return alert(
          `A imagem da resposta incorreta 2 da pergunta ${
            page + 1
          } deve ter formato de URL`
        );
      }
    ...
    if (array[page].quizzIncorrectAnswer3 !== "") {
      if (!CheckURL(array[page].quizzIncorrectAnswerURL3)) {
        return alert(
          `A imagem da resposta incorreta 3 da pergunta ${
            page + 1
          } deve ter formato de URL`
        );
      }
...
const CheckHexa = (hex) =>
  hex === "" || hex.includes("#") === false || hex.length !== 7 ? true : false;

let currentLevel = 1;

function RenderLevels(page, number) {
  const LevelsInputContainer = document.querySelector(
    ".quizz-levels-input-container"
  );
...
function CheckLevelQuizzData(page, array) {
  if (array[page] === null || array[page] === undefined) {
    alert(
      "Voce precisa preencher as informacoes de todas os niveis antes de continuar"
    );
    return false;
  } else {
    if (array[page].quizzLevel === "" || array[page].quizzLevel.length < 10) {
      return alert(
        `O texto do nivel ${page + 1} deve ter no mínimo 10 caracteres`
      );
    }
    if (
      array[page].LevelPorcent === "" ||
      array[page].LevelPorcent < 0 ||
      array[page].LevelPorcent > 100
    ) {
      return alert(
        `A porcentagem de acerto minimo do nivel ${
          page + 1
    ...
    if (
      array[page].LevelDescription === "" ||
      array[page].LevelDescription.length < 30
    ) {
      return alert(
        `A descrição do nivel ${page + 1} deve ter no mínimo 30 caracteres`
      );
    }
  }
  if (currentPage === numberOfLevels) {
...
function GenerateUserRequestPost(questions, levels) {
  ...
  let requestUserQuizz = axios.post(
  ...
  requestUserQuizz.then((response) => {
    ...
    window.localStorage.setItem(
      `UserQuizzID${TOTAL_USER_QUIZZ}`,
      `${response.data.id}`
    );
    window.localStorage.setItem("TOTAL_USER_QUIZZ", `${TOTAL_USER_QUIZZ}`);
    console.log(
      `voce criou o quizz id ${response.data.id} com extremo sucesso`
    );
    ...
    document
      .querySelector(".quizz-success-box")
      .setAttribute("onclick", `getOnlyQuizz(${response.data.id})`);
    console.log("opa");
  });
